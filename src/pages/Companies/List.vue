      ...
      <template #body-cell-action="{ row: { _id } }">
        <td class="text-center">
          <q-btn
            ...
            round
            icon="edit"
            @click="$router.push(`/companies/edit/${_id}`)"
...
export default {
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { field: row => row.groupInfo.uen || '', name: 'groupInfo.uen', label: 'UEN', align: 'left', sortable: true },
        ...
        { field: row => row.organizationType.name, name: 'organizationType', label: 'Organization Type', align: 'left' },
        { field: row => row.parent.name, name: 'parent', label: 'Group', align: 'left' },
  ...
  methods: {
    ...
    async onRequest ({ pagination }) {
      ...
      this.findRoles({
        query: {
          $populate: ['organizationType', 'parent'],
          $limit: rowsPerPage,
    ...
    async getFilterValue () {
      const string = ['name', 'groupInfo.uen', 'groupInfo.email', 'groupInfo.address', 'groupInfo.postalCode', 'groupInfo.phone']
      const options = ['parent', 'organizationType', 'status']
      const filter = {}
