    ...
    >
      <template #body-cell-action="{ row: { _id } }">
        <td class="text-center">
          <q-btn
            ...
            round
            icon="edit"
            @click="$router.push(`/license-types/edit/${_id}`)"
            v-if="$can('update', 'License Types')"
          >
          ...
          <q-btn
            ...
            class="q-ml-sm"
            @click="onDelete(_id)"
            v-if="$can('delete', 'License Types')"
          >
            <q-tooltip>Delete</q-tooltip>
          </q-btn>
        </td>
      </template>
      <template #body-cell-tag="{ row: { tag } }">
        <td class="text-center">
          <q-badge
            v-if="tag"
            color="teal"
      ...
      <template #top>
        <q-btn
          ...
          text-color="red-1"
          label="Add"
          @click="$router.push('/license-types/add')"
          v-if="$can('create', 'License Types')"
        />
      </template>
    </q-table>
  </div>
  ...
  data () {
    return {
      ...
      columns: [
        ...
        { field: 'name', name: 'name', label: 'License Name', align: 'left', sortable: true },
        { field: row => row.status ? 'Active' : 'Inactive', name: 'status', label: 'Status', align: 'left', sortable: true },
        { name: 'tag', label: 'Tag', align: 'center' }
      ],
      data: [],
      pagination: {
        page: 1,
