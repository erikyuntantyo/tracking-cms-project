<template>
  <div>
    ...
    >
      <template #body-cell-action="{ row: { _id } }">
        <td class="text-center">
          <q-btn
            ...
            round
            icon="edit"
            @click="$router.push(`/cdas-users/edit/${_id}`)"
            v-if="$can('update', 'CDAS Users')"
          >
      ...
      <template #top>
        <q-btn
          v-if="$can('create', 'CDAS Users')"
          padding="sm lg"
          unelevated
          color="red-10"
          text-color="red-1"
          label="Add"
          @click="$router.push('/cdas-users/add')"
        />
      </template>
    </q-table>
  </div>
